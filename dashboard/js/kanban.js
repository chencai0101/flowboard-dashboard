...
function renderAddTaskForm() {
  return `<div class="add-task-form">
    ...
    <div class="form-row">
      <label for="newTaskDueDate" style="font-size:11px;color:var(--text-muted);">截止日期</label>
      <input id="newTaskDueDate" type="date" style="border:1px solid var(--border);border-radius:4px;padding:2px 6px;font-size:12px;background:var(--bg-secondary);color:var(--text);width:140px;">
    </div>
...
export async function createTask(state) {
  ...
  const title = inp ? inp.value.trim() : '';
  if (!title) return;
  const dueDateInp = document.getElementById('newTaskDueDate');
  const dueDate = dueDateInp && dueDateInp.value ? dueDateInp.value : null;
  const body = { title, priority: kanbanState.selectedPriority };
